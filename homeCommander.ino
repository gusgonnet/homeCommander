...
void dryer_status_old() {

  // Check if we need to start the next sample
  if (millis() > DHTnextSampleTime) {

    Particle.publish(APP_NAME, "starting DHT22 sample", 60, PRIVATE);
    ...
    if (!DHT.acquiring()) {		// has sample completed?
      ...
      Particle.publish(APP_NAME, "sample acquired", 60, PRIVATE);
...
//call this function when the algorithm gets stuck (For instance, when you manually stopped the dryer cycle)
...
Particle.publish(APP_NAME, "starting publish temp", 60, PRIVATE);
